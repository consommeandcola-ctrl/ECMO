...
const LEGACY_SRC = path.join(__dirname, "..", "ecpr_commander_v6.0_legacy.txt");
const LEGACY_HTML = path.join(__dirname, "..", "ecpr_commander_v6.0_legacy.html");
...
test.beforeEach(async ({ page }) => {
  await page.goto("/ecpr_commander_v6.0_legacy.html");
  await page.evaluate(() => {
    localStorage.clear();
...
test("初期表示: CPAタブ・タイマー未開始", async ({ page }) => {
  ...
  await expect(page).toHaveTitle(/Legacy/);
});
...
test("蘇生処置: 除細動・アドレナリン・波形・ROSC", async ({ page }) => {
  await fillCpaScenario(page);
  await page.locator("#btnConfirmCpa").click();
...
test("カニューレモーダル: 体重レンジ選択と留置記録", async ({ page }) => {
  ...
  await expect(page.locator("#art_cann_size option")).toHaveCount(7);
  await expect(page.locator("#art_cann_size option").nth(1)).toHaveText("14Fr 15cm");
...
test("心肺蘇生報告書: 構造・セクション・タイムスタンプ", async ({ page }) => {
  ...
  expect(report).toContain("最終波形: ROSC");
...
test("目撃あり/なしは排他選択", async ({ page }) => {
  ...
  const state = await page.evaluate(() => JSON.parse(localStorage.getItem("ecpr_commander_v6_legacy")).cpa.witness);
...
test("レガシーUI: 記号はASCII", async ({ page }) => {
  await expect(page.locator('[data-step-count="epi"][data-delta="-1"]')).toHaveText("-");
  await expect(page.locator('[data-step-count="epi"][data-delta="1"]')).toHaveText("+");
  await page.locator(".kit-head").first().click();
  await expect(page.locator(".kit-head").first().locator("span").first()).toHaveText("v");
});

test("レガシーUI: 時刻欄に全面オーバーレイCSSなし", async ({ page }) => {
  ...
  expect(style).not.toMatch(/calendar-picker-indicator.*width:100%/);
});

test("レガシーUI: モーダル閉じるボタンはASCII", async ({ page }) => {
  const closeTexts = await page.evaluate(() =>
    [...document.querySelectorAll(".modal-h button[data-close]")].map((b) => b.textContent)
  );
  expect(closeTexts.length).toBeGreaterThan(0);
  expect(closeTexts.every((t) => t === "x")).toBe(true);
});
